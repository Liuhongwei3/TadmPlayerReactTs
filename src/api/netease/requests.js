...
const hotDetails = async (cat = "全部", limit = 24) => {
    let flag = await doReq(`/top/playlist?cat=${cat}&limit=${limit}`);
    if (!flag) {
        return [];
    }
...
const hotDetailCats = async () => {
    let flag = await doReq(`/playlist/catlist`);
    if (!flag) {
        return [];
    }
